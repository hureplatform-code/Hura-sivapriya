  ...
  return (
    <DashboardLayout>
      <div className="space-y-8">
        <div>
          <h1 className="text-2xl font-semibold text-slate-900 tracking-tight">Growth & Add-ons</h1>
          ...
          {addOns.map((add, i) => (
            ...
            >
              <div className="absolute top-0 right-0 p-4">
                <span className={`px-3 py-1 rounded-full text-[10px] font-semibold uppercase tracking-widest ${add.status === 'active' ? 'bg-emerald-50 text-emerald-600' : 'bg-primary-50 text-primary-600'}`}>
                  {add.status}
                </span>
              </div>
              ...
              <h3 className="text-xl font-semibold text-slate-900">{add.name}</h3>
              <p className="text-2xl font-semibold text-slate-900 mt-2">{APP_CONFIG.CURRENCY} {add.price} <span className="text-xs font-medium text-slate-400 uppercase tracking-widest">/mo</span></p>
              
              <button className={`w-full mt-8 py-4 rounded-2xl font-medium text-xs uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${add.status === 'active' ? 'bg-slate-50 text-slate-400 cursor-not-allowed' : 'bg-primary-600 text-white shadow-lg shadow-primary-100 hover:bg-primary-700'}`}>
