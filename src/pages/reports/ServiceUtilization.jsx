...
export default function ServiceUtilization() {
  ...
  if (loading) return <DashboardLayout><div className="min-h-screen flex items-center justify-center bg-slate-50 py-12 px-4 sm:px-6 lg:px-8 text-center font-medium text-slate-500">Calculating Utilization Stats...</div></DashboardLayout>;
    ...
    <DashboardLayout>
      <div className="space-y-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-semibold text-slate-900 tracking-tight">Service Utilization</h1>
            <p className="text-slate-500 mt-1">Cross-departmental analytics of service consumption and hospital traffic.</p>
          </div>
          <div className="flex gap-3">
             <button className="flex items-center gap-2 px-6 py-3 bg-white text-slate-900 font-medium rounded-2xl border border-slate-200 hover:bg-slate-50 shadow-sm">
              <Calendar className="h-5 w-5" />
              All Time
            </button>
            <button className="flex items-center gap-2 px-6 py-3 bg-primary-600 text-white font-medium rounded-2xl hover:bg-primary-700 transition-all shadow-lg shadow-primary-200 active:scale-95">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  {departments.map((item, i) => (
                    <div key={i} className="flex-1 flex flex-col items-center gap-4">
                      <div className="w-full relative group/bar">
                        ...
                        />
                        <div className="absolute -top-10 left-1/2 -translate-x-1/2 bg-slate-900 text-white text-[10px] px-2 py-1 rounded-lg opacity-0 group-hover/bar:opacity-100 transition-opacity whitespace-nowrap font-medium">
                          {item.usage} Traffic
                        </div>
                      </div>
                      <span className="text-[10px] font-medium text-slate-400 uppercase tracking-widest text-center truncate w-full">{item.name}</span>
                    </div>
                  ))}
               </div>
             </div>
                ...
                {departments.map((service, i) => (
                  ...
                  >
                    ...
                    <div className={`flex items-center gap-1 ${service.trend === 'up' ? 'text-emerald-500' : 'text-red-500'}`}>
                      <span className="text-xs font-medium">{service.trend === 'up' ? '+' : '-'}{Math.floor(Math.random() * 15)}%</span>
          ...
          <div className="space-y-8">
            <div className="bg-slate-900 p-8 rounded-3xl text-white relative overflow-hidden">
               <div className="flex items-center justify-between mb-8">
                  <h3 className="font-medium text-white/90 text-sm uppercase tracking-widest">Peak Utilization</h3>
                  <Activity className="h-5 w-5 text-primary-400" />
               </div>
               <p className="text-3xl font-semibold italic">{peakTraffic}</p>
               <p className="text-white/40 text-[10px] font-semibold uppercase tracking-widest mt-2">Daily Traffic Zenith</p>
               
               <div className="mt-8 space-y-4">
                  <div className="flex items-center justify-between p-3 bg-white/5 rounded-2xl border border-white/5">
                    <span className="text-xs font-medium text-white/60">Weekday Avg</span>
                    <span className="text-xs font-medium text-emerald-400">88%</span>
                  </div>
                  <div className="flex items-center justify-between p-3 bg-white/5 rounded-2xl border border-white/5">
                    <span className="text-xs font-medium text-white/60">Weekend Avg</span>
                    <span className="text-xs font-medium text-amber-400">42%</span>
                  </div>
               </div>
            ...
            <div className="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm">
                ...
                <div className="relative h-48 w-48 mx-auto">
                   ...
                   <div className="absolute inset-0 flex flex-col items-center justify-center">
                      <span className="text-[10px] font-medium text-slate-400 uppercase">Consulting</span>
                      <span className="text-lg font-semibold text-slate-900">72%</span>
                   </div>
                </div>
            </div>
          </div>
