...
export default function DiagnosisTrend() {
  ...
  if (loading) return <DashboardLayout><div className="min-h-screen flex items-center justify-center bg-slate-50 py-12 px-4 sm:px-6 lg:px-8 text-center font-medium text-slate-500">Analyzing Clinical Data...</div></DashboardLayout>;
  
  if (error) return (
    <DashboardLayout>
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 py-12 text-center">
        <div className="text-red-500 mb-4 text-xl font-semibold">Error Loading Report</div>
    ...
    <DashboardLayout>
      <div className="space-y-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-semibold text-slate-900 tracking-tight">Diagnosis Trends</h1>
            <p className="text-slate-500 mt-1">Epidemiological analysis and disease prevalence tracking.</p>
          </div>
          <div className="flex gap-3">
            <button className="flex items-center gap-2 px-6 py-3 bg-white text-slate-600 font-medium rounded-2xl border border-slate-100 shadow-sm">
              <Filter className="h-5 w-5" />
              This Year
            </button>
            <button 
              onClick={() => setShowComparative(true)}
              className="flex items-center gap-2 px-6 py-3 bg-slate-900 text-white font-medium rounded-2xl hover:bg-slate-800 transition-all shadow-lg active:scale-95"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          <div className="lg:col-span-3 space-y-8">
            <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
              <h3 className="text-lg font-semibold text-slate-900 mb-8">Top 5 Conditions (Monthly Trend)</h3>
              <div className="space-y-8">
                {trends.length > 0 ? trends.map((t, i) => (
                  <div key={t.disease} className="space-y-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <span className="text-sm font-medium text-slate-700">{t.disease}</span>
                        <div className={`flex items-center text-[10px] font-semibold uppercase ${t.trend === 'up' ? 'text-red-500' : 'text-emerald-500'}`}>
            ...
            <div className="bg-slate-900 rounded-[2.5rem] p-10 text-white relative overflow-hidden group">
              <div className="relative z-10 flex flex-col md:flex-row gap-8 items-center">
                ...
                <div className="flex-1 text-center md:text-left">
                  <h3 className="text-2xl font-semibold">Clinical Early Warning System</h3>
          ...
          <div className="space-y-6">
            <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm text-center">
              ...
              <h4 className="text-xl font-semibold text-slate-900">Prevention Rate</h4>
              <p className="text-2xl font-semibold text-emerald-600 mt-2">{stats.prevRate}</p>
              <p className="text-xs text-slate-400 font-semibold uppercase mt-1 tracking-widest">Follow-up Success</p>
              <div className="mt-8 pt-8 border-t border-slate-50 flex items-center justify-between">
                <div className="text-left">
                  <p className="text-sm font-medium text-slate-900">{stats.screened}</p>
                  <p className="text-[10px] text-slate-400 font-semibold uppercase">Screened</p>
                </div>
                <div className="text-right">
                  <p className="text-sm font-medium text-slate-900">{stats.improved}</p>
                  <p className="text-[10px] text-slate-400 font-semibold uppercase">Improved</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
              ...
              <div className="space-y-3">
                {['All Conditions', 'Chronic Diseases', 'Acute Infections', 'Pediatric Trends'].map((tag) => (
                  <button key={tag} className="w-full text-left px-5 py-3.5 bg-slate-50 hover:bg-slate-100 rounded-2xl text-sm font-medium text-slate-600 transition-all">
                    {tag}
                  </button>
                ))}
              </div>
      ...
      {showComparative && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-md">
          <div className="bg-white rounded-[2.5rem] p-10 max-w-2xl w-full shadow-2xl relative">
            ...
            <h3 className="text-2xl font-semibold text-slate-900 mb-2">Comparative Epidemiological Analysis</h3>
            ...
            <div className="space-y-4">
              {trends.map(t => (
                <div key={t.disease} className="p-6 bg-slate-50 rounded-3xl flex items-center justify-between">
                  <div>
                    <p className="font-medium text-slate-900">{t.disease}</p>
                    <p className="text-xs text-slate-500 font-medium">Prev: {t.previous} cases • Curr: {t.current} cases</p>
                  </div>
                  <div className={`flex items-center gap-2 font-medium text-sm ${t.trend === 'up' ? 'text-red-500' : 'text-emerald-500'}`}>
            ...
            <button 
              ...
              className="w-full mt-10 py-4 bg-slate-900 text-white rounded-2xl font-medium text-xs uppercase tracking-widest hover:bg-slate-800 transition-all shadow-xl"
            >
              Close Report
            </button>
          </div>
