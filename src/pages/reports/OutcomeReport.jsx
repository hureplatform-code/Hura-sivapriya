...
export default function OutcomeReport() {
  ...
  if (loading) return <DashboardLayout><div className="min-h-screen flex items-center justify-center bg-slate-50 py-12 px-4 sm:px-6 lg:px-8 text-center font-medium text-slate-500">Analyzing Clinical Outcomes...</div></DashboardLayout>;
    ...
    <DashboardLayout>
      <div className="space-y-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <button className="flex items-center gap-2 px-6 py-3 bg-white text-slate-900 font-medium rounded-2xl border border-slate-200 hover:bg-slate-50 transition-all shadow-sm active:scale-95">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm">
            <div className="flex items-center justify-between mb-8">
              ...
              <select className="bg-slate-50 border-none rounded-xl text-xs font-medium px-3 py-1.5 outline-none">
              ...
              {outcomes.length > 0 ? outcomes.map((doc, i) => (
                <div key={i} className="space-y-2">
                  <div className="flex justify-between items-center text-sm">
                    <span className="font-medium text-slate-700">{doc.consultant} ({doc.patients} cases)</span>
                    <span className="font-medium text-primary-600">{doc.recoveryRate}</span>
                  </div>
          ...
          <div className="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm">
            <h3 className="font-medium text-slate-900 flex items-center gap-2 mb-8">
            ...
            <div className="flex flex-col items-center justify-center h-[200px] border-2 border-dashed border-slate-100 rounded-2xl relative group">
                ...
                <p className="text-slate-400 text-xs font-medium mt-4">Consultant Satisfaction Metrics tracked per visit.</p>
