            ...
            >
               ...
               <div className="p-8 max-h-[80vh] overflow-y-auto">
                  ...
                  <div className="mt-10 space-y-8">
                    ...
                    {/* MODULES */}
                    <div className="pt-6 border-t-2 border-slate-50">
                       <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4">Module Management</label>
                       <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                          <button
                            type="button"
                            onClick={() => setEditForm({
                              ...editForm,
                              modules: { ...editForm.modules, ward: !editForm.modules?.ward }
                            })}
                            className={`flex flex-col items-start gap-2 p-3.5 rounded-2xl border-2 transition-all
                              ${editForm.modules?.ward !== false ? 'bg-indigo-50 border-indigo-200 text-indigo-900' : 'bg-slate-50 border-transparent text-slate-400 md:grayscale'}
                            `}
                          >
                             <div className={`h-5 w-5 rounded-full border-2 flex items-center justify-center ${editForm.modules?.ward !== false ? 'bg-indigo-600 border-indigo-600' : 'bg-white border-slate-200'}`}>
                                {editForm.modules?.ward !== false && <div className="h-1.5 w-1.5 bg-white rounded-full" />}
                             </div>
                             <span className="text-[9px] font-black uppercase tracking-wider">Ward Management</span>
                          </button>
                       </div>
                    </div>
                  </div>
