    ...
    <DashboardLayout>
      <div className="space-y-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-semibold text-slate-900 tracking-tight">Procedure Master</h1>
          ...
          <div className="flex bg-slate-100 p-1.5 rounded-2xl">
            <button 
              ...
              className={`px-6 py-2 rounded-xl text-sm font-medium transition-all ${activeTab === 'dental' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
            >
              Dental Procedures
            </button>
            <button 
              onClick={() => setActiveTab('other')}
              className={`px-6 py-2 rounded-xl text-sm font-medium transition-all ${activeTab === 'other' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
            >
              Other Clinical
            </button>
          </div>
        ...
        <div className="max-w-4xl mx-auto space-y-6">
                ...
                {filtered.map((proc) => (
                  ...
                  >
                    <div className="flex items-center gap-6">
                       ...
                       <div>
                          <p className="font-medium text-slate-900 text-lg">{proc.name}</p>
                          <p className="text-xs font-medium text-slate-400 uppercase tracking-wider">{proc.group} Department</p>
                       </div>
                    </div>
                    <div className="flex items-center gap-6">
                       <div className="text-right">
                          <p className="text-lg font-semibold text-primary-600">${proc.price}</p>
                          <p className="text-[10px] font-medium text-slate-300 uppercase tracking-widest">Base Rate</p>
                       </div>
          ...
          <div className="flex items-center gap-3 justify-center text-[10px] font-medium text-slate-300 uppercase tracking-widest pt-4">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
            ...
            >
              <div className="p-8 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
                <h2 className="text-2xl font-semibold text-slate-900">{editingItem ? 'Update' : 'Add'} Procedure</h2>
              ...
              <form onSubmit={handleSave} className="p-8 space-y-6">
                <div className="space-y-2">
                  <label className="text-[10px] font-medium text-slate-400 uppercase tracking-widest ml-1">Procedure Name</label>
                  <input 
                    required
                    type="text"
                    value={newItem.name}
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-medium text-slate-400 uppercase tracking-widest ml-1">Standard Price ($)</label>
                  <input 
                    required
                    type="number"
                    value={newItem.price}
