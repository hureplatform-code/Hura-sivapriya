    ...
    <DashboardLayout>
      <div className="space-y-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-semibold text-slate-900 tracking-tight">Clinical Note Setup</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          {templates.map((temp, i) => (
            ...
            >
              ...
              <h3 className="font-medium text-slate-900 group-hover:text-primary-600 transition-colors">{temp.name}</h3>
              <div className="flex items-center gap-2 mt-2">
                <span className="text-[10px] font-medium text-slate-300 uppercase tracking-widest">{temp.type}</span>
                <span className="h-1 w-1 rounded-full bg-slate-200" />
                <span className="text-xs font-medium text-slate-500">{temp.blocks || 0} Data Blocks</span>
              </div>

              <div className="mt-8 flex items-center justify-between">
                <span className={`px-2 py-1 rounded-lg text-[10px] font-semibold uppercase tracking-widest ${temp.status === 'Active' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'}`}>
                  {temp.status || 'Active'}
                </span>
        ...
        <div className="bg-slate-50 rounded-3xl p-8 border-2 border-dashed border-slate-200 flex flex-col items-center justify-center text-center">
          ...
          <h4 className="font-medium text-slate-900">Custom Data Blocks</h4>
          ...
          <button className="mt-6 px-6 py-2.5 bg-white text-slate-900 font-medium text-sm rounded-xl border border-slate-100 shadow-sm hover:shadow-md transition-all">
            Manage Blocks
          </button>
        </div>
      </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
            ...
            >
              <div className="p-8 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
                <h2 className="text-2xl font-semibold text-slate-900">{editingItem ? 'Update' : 'Create'} Template</h2>
              ...
              <form onSubmit={handleSave} className="p-8 space-y-6">
                <div className="space-y-2">
                  <label className="text-[10px] font-medium text-slate-400 uppercase tracking-widest ml-1">Template Name</label>
                  <input 
                    required
                    type="text"
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-medium text-slate-400 uppercase tracking-widest ml-1">Type</label>
                  <select 
